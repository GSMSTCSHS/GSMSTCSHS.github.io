...
const SideBar = () => {
  ...
  return (
    <div className="flex sm:hidden sticky top-0 shadow-xl w-full bg-cshs-800 justify-start items-start font-display py-1">
      <div className="flex flex-col items-center w-full">
        ...
        <div className="flex flex-row w-full items-center justify-between px-3">
          <div className="flex flex-row gap-3 items-center">
              ...
              <p className="text-cshs-300 text-xl">
                GSMST CSHS
              </p>
          ...
          <div className="flex flex-row gap-3 items-center  text-cshs-300">
            <button onClick={closeDrawer}>
              <Menu height="35px" width="px"  color="text-cshs-300" className= {`${closedDrawer ? '' : 'hidden'}`}/>
              <CloseOutline height="35px" width="px"  color="text-cshs-300" className= {`${closedDrawer ? 'hidden' : ''}`}/>
            </button>
            <Link href={"/account"} className="items-center mx-auto">
              <PersonCircleOutline height="35px" width="px" 
            style={{ fill: "var(--color-cshs-300)" }}/>
            </Link>
          </div>
        </div>
        <div className= {`${closedDrawer ? 'hidden' : ''} border border-cshs-300 rounded-full h-0 w-[95vw] my-1`} />
          
        <div className = {`${closedDrawer ? 'hidden' : ''} w-full flex flex-col pb-3 items-start pl-3 gap-2`}>
            {pages.map((value) => (
            <Link href={value.href} key={value.label} className={`${closedDrawer ? 'hidden' : ''} md:text-lg lg:text-xl text-cshs-300`}>
                {value.label}
            </Link>
            ))}
        </div>
